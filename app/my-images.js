...
export default function MyImages() {
   ...
   const handleShare = async (imageUrl) => {
       try {
           ...
           // Download the image
           const downloadPath = `${FileSystem.cacheDirectory}temp_image.jpg`;
           const { uri: localUri } = await FileSystem.downloadAsync(imageUrl, downloadPath);
   ...
   const renderItem = ({ item, index }) => {
       if (isGridView) {
           return (
               <View style={[s.gridItem, index % 2 === 0 ? { marginRight: 4 } : { marginLeft: 4 }]}>
                   <Image 
                       source={{ uri: item.image_url }} // Changed from item.url to item.image_url
                       style={s.gridImage}
                       resizeMode="cover"
                   />
                   <View style={s.gridOverlay}>
                       ...
                       <TouchableOpacity
                           style={s.iconButton}
                           onPress={() => handleShare(item.image_url)} // Changed from item.url
                       >
       ...
       return (
           <View style={s.listItem}>
               <Image 
                   source={{ uri: item.image_url }} // Changed from item.url
                   style={s.listImage}
                   resizeMode="cover"
               />
               <View style={s.listContent}>
                   <Text style={s.promptText} numberOfLines={2}>{item.prompt}</Text>
